...
public:
    ...
    // tree
    const int    NUM_EXCLUDE_RECENT = 500; // simply just keyframe gap, but node position distance-based exclusion is ok. 
    ...
    const double SC_DIST_THRES = 0.11; // 0.4-0.6 is good choice for using with robust kernel (e.g., Cauchy, DCS) + icp fitness threshold
